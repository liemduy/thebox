...
function displayDate(ymd, long = false) {
  ...
  if (long) return date.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric"
  });
...
function parseRouteHash(hash = window.location.hash) {
  ...
  if (name === "actions") return {
    name: "actions",
    ui: parseActionRouteParams(params)
  };
  if (name === "search") {
...
function buildAppHash({
  ...
  if (isSearchOpen) {
    ...
    if (currentView === "actions") appendActionRouteParams(params, ui);else appendBoxRouteParams(params, ui);
...
function RichNoteModal({
  ...
  function wrapBlock(tagName) {
    ...
    const el = document.createElement(tagName);
    if (range.collapsed) el.textContent = tagName === "h3" ? "Heading" : "Quote";else el.appendChild(range.extractContents());
  ...
  return React.createElement("div", {
    className: "fixed inset-0 z-50 flex items-center justify-center p-4 pb-28 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200",
    onClick: onClose
  }, React.createElement("div", {
  ...
  }, isBoxNote ? "Box notes" : modal.entryId ? "Edit note" : "Add note"), React.createElement("div", {
    ...
    className: "fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[340px] bg-[#232323] border border-[#3E3E3E] rounded-[14px] px-5 py-3.5 flex items-center justify-between shadow-2xl z-50"
  }, React.createElement("div", {
    className: "flex gap-4 text-[#A7A7A7]"
  }, React.createElement("button", {
    type: "button",
    onClick: () => applyFormat("bold"),
    className: "hover:text-[#FFD2D7] transition-colors"
...
function App() {
    ...
    if (sb && userId) {
      try {
        ...
        if (!stateError && stateRow?.data) {
          ...
          next = shouldPreferLocal(localState, cloudState, cloudUpdatedAt) ? markPendingSync(localState, localState?.meta?.localUpdatedAt || now()) : cloudState;
  ...
  }, [currentView, isSearchOpen, searchQuery, db.ui.boxView, db.ui.boxFilter, db.ui.boxFilterFrom, db.ui.boxFilterTo, db.ui.showBoxDays, db.ui.selectedActionDate, db.ui.actionFilter]);
  useEffect(() => {
    if (!flashTarget) return;
    const safeId = window.CSS?.escape ? window.CSS.escape(flashTarget.id) : String(flashTarget.id).replace(/"/g, '\\"');
    const selector = flashTarget.type === "entry" ? `[data-action-entry-id="${safeId}"]` : flashTarget.type === "action" ? `[data-action-node-id="${safeId}"]` : `[data-box-node-id="${safeId}"]`;
  ...
  function saveActionNote({
    ...
    commit("Save action note", state => {
      ...
      if (entry) {
        ...
        node.entries.push(normalizeEntry({
          type: "note",
          title: title || "Note",
          bodyHtml,
          createdAt: t,
  ...
  return React.createElement("div", {
    ...
    className: "view-title text-[2.5rem] leading-[1.1] font-extrabold tracking-tighter"
  }, React.createElement("button", {
    type: "button",
  ...
  }, "No items match this filter.")))), modal?.type === "boxNote" && React.createElement(RichNoteModal, {
    modal: modal,
    state: db,
